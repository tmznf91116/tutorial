...
$(document).ready(function(){
	...
	$("#modify").click(function(){//[회원정보수정]버튼 클릭시 이 메소드 수행
		var query = {passwd:$("#passwd").val()};
		
		$.ajax({
			type: "post",
			url: "memberCheck.jsp",// 이파일이 실행 후  success 실행
			data: query,
			success: function(data){
				if(data == 1)//비밀번호가 맞음
		    		 $("#main_auth").load("modifyForm.jsp?passwd="+$("#passwd").val());	//? 쿼리스트링 modifyForm.jsp 실행할때 실행함
		    	//현재 가지고 있던 내용을 부분업데이트 하는것이 ajox main_auth에 새로운 내용이 담김
				
				//"logincheck.jsp?
				//userid=" $('#userid').val() + "&passwd=" +$('#passwd').val()";

				//logincheck.jsp?userid=doccomsa&passwd=1111;
				else {//비밀번호 틀림
		    	  	 alert("비밀번호가 맞지 않습니다.");
		    	  	 $("#passwd").val("");
		    	  	 $("#passwd").focus();
